...
$rubricCounts = []; // To hold the aggregated counts
...
while ($row = $result_rubric_counts->fetch_assoc()) {
    $description = $row['deskripsi_rubrik'];
    $count = $row['total_students'];

    if (!isset($rubricCounts[$description])) {
        $rubricCounts[$description] = 0; // Initialize if not set
    }
    $rubricCounts[$description] += $count; // Aggregate counts
}

// Prepare data for the chart
$combinedDescriptions = array_keys($rubricCounts);
$combinedCounts = array_values($rubricCounts);
            ...
            <div class="col-md-6">
                <div class="card mb-4">
                    <div class="card-header" style="background-color: #a0c1d9;">
                        <h6 class="card-title text-center">Carta Jumlah Pelajar Berdasarkan PBD</h6>
                    </div>
                    <div class="card-body">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const combinedChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: <?php echo json_encode($combinedDescriptions); ?>, // Combined descriptions
            datasets: [{
                label: 'Jumlah Pelajar Berdasarkan Deskripsi Rubrik',
                data: <?php echo json_encode($combinedCounts); ?>, // Combined counts
