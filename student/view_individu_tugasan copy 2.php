...
<div class="mt-3 mb-4">
        ...
        <?php else: ?>
            ...
            <tr>
                ...
                <tr>
                    <th>Status Penyerahan 1</th>
                    <td>
                        Telah Submit pada : <?= date("d F Y", strtotime($submission_info['tarikh_penyerahan1'])) ?><br>
                        File: <a target="_blank" href="<?= $submission_info['penyerahan_path1'] ?>"><?= basename($submission_info['penyerahan_path1']) ?></a><br>
                        Komen: <?= !empty($submission_info['komen']) ? $submission_info['komen'] : 'Belum ada komen'; ?>
                    </td>
                </tr>
                <tr>
                    <th>Status Penyerahan 1</th>
            ...
            <?php else: // If there is a comment from the teacher ?>
                ...
                <?php else: ?>
                    <table class="table table-striped table-bordered">
                        <tr>
                            <th>Status Penyerahan 2</th>
                            <td>
                                Telah Submit pada : <?= date("d F Y", strtotime($submission_info['tarikh_penyerahan2'])) ?><br>
                                File: <a target="_blank" href="<?= $submission_info['penyerahan_path2'] ?>"><?= basename($submission_info['penyerahan_path2']) ?></a><br>
                                Gred: <?= !empty($submission_info['nama_rubrik']) ? $submission_info['nama_rubrik'] : 'Belum dinilai'; ?>
                            </td>
                        </tr>
                    </table>

                    <?php if (!empty($submission_info['penyerahan_path2'])): ?>
                        <?php if (empty($submission_info['url'])): ?>
                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addSubmission3Modal">Hantar Video Presentation (Pilihan)</button>
                        <?php else: ?>
                            <table class="table table-striped table-bordered">
                                <tr>
                                    <th>Status Video Presentation</th>
                                    <td>
                                        URL: <a target="_blank" href="<?= $submission_info['url'] ?>"><?= $submission_info['url'] ?></a>
                                    </td>
                                </tr>
                            </table>
                        <?php endif ; ?>
