...
function getTotalStudents($conn, $tugasan_id) {
  $sql = "SELECT COUNT(*) as total_students FROM pelajar";
  $result = $conn->query($sql);
  $row = $result->fetch_assoc();
  return $row["total_students"];
}

function getSubmittedStudents($conn, $tugasan_id) {
  $sql = "SELECT COUNT(*) as submitted_students FROM penyerahan WHERE tugasan_id = $tugasan_id AND penyerahan_path IS NOT NULL";
  $result = $conn->query($sql);
  $row = $result->fetch_assoc();
  return $row["submitted_students"];
}
?>
            ...
            <div class="col-md-6 mb-3">
                <div class="card h-100">
                    <div class="card-body">
                        ...
                        <?php $total_students = getTotalStudents($conn, $row_tugasan["tugasan_id"]); $submitted_students = getSubmittedStudents($conn, $row_tugasan["tugasan_id"]);
                        ?>
                        <p class="card-text "> <?= $submitted_students ?>/<?= $total_students ?> Pelajar telah selesai hantar</p>
